...
export async function runInitWizard(
  ...
  try {
    ...
    await createAgentsMd(cwd, editor, selectedSkills, repoPath);
    ...
    // Done!
    printSuccess({ skillsPath, agentsPath });
...
async function createAgentsMd(
  ...
  const editorConfig = EDITORS[editor];
  const templatePath = join(repoPath, "bootstrap", "templates", "AGENTS.template.md");
  ...
  try {
    ...
    content = createFallbackAgentsMd(editorConfig, skills);
    await writeFile(agentsMdPath, content, "utf-8");
    return;
  }

  // Replace skills/ and agents/ URLs with editor-specific paths
  content = content.replace(/\(skills\//g, `(${editorConfig.skillsPath}/`);
  content = content.replace(/\(agents\//g, `(${editorConfig.agentsPath}/`);

  // Filter skill tables to show only installed skills
  content = filterSkillTables(content, skills);

  await writeFile(agentsMdPath, content, "utf-8");
}

function createFallbackAgentsMd(
  editorConfig: { name: string; skillsPath: string; agentsPath: string },
  skills: string[]
): string {
  return `# AGENTS.md
...
function filterSkillTables(content: string, installedSkills: string[]): string {
  // Match markdown tables that contain skill references
  // Tables have headers like: | Skill | Description | URL | or | Skill | Description | Status | URL |
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Detect table header with "Skill" column
    if (line.includes("| Skill") && line.includes("|")) {
      inSkillTable = true;
      headerLine = line;
    ...
    // If in a skill table, filter rows
    if (inSkillTable) {
      // Check if this is still a table row
      if (line.startsWith("|") && line.endsWith("|")) {
        // Extract skill name from first column (format: | `skill-name` | ...)
        const skillMatch = line.match(/\|\s*`([^`]+)`\s*\|/);
        if (skillMatch) {
          const skillName = skillMatch[1];
          // Keep row only if skill is installed
          if (skillName && installedSkills.includes(skillName)) {
            result.push(line);
          }
        } else {
          // Row doesn't match skill pattern, keep it (might be template placeholder)
          result.push(line);
        }
      } else {
        // End of table
